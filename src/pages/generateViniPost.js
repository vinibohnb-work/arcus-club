// ── Gerador de HTML para posts do calendário editorial — Vinícius Bohn ────────
...
const FMT_MAP = {
  ...
  Posicionamento: { bg: "#1A0505", border: "#8A1A1A", light: "#D55B5B", label: "POSICIONAMENTO" },
};

function readBrand() {
  try { return { ...BRAND_FALLBACK, ...(JSON.parse(localStorage.getItem("vini-brand")) || {}) }; }
...
export function generatePrompt(post) {
  ...
  const fmtSlides = { Carrossel: n, "Estático": 1, Reels: 5, Bastidor: 1, Posicionamento: 1 };
  const fmtDesc   = {
    ...
    Posicionamento: "post de posicionamento para Instagram",
  };
...
`;
  ...
  // ── POSICIONAMENTO ─────────────────────────────────────────────────────────
    ...
    specific =
`
━━━ CONTEÚDO DO SLIDE ÚNICO (POSICIONAMENTO) ━━━
1 slide único, 1080×1350px. Contador "01/01".
Post de posicionamento pessoal. Tom: autoridade + proximidade. Não vende — posiciona.

.content (position relative, z-index 10, padding 130px 80px, height 100%, display flex, flex-direction column, justify-content center):

  Topo: label "Posicionamento · ${post.pilar}"
        (Jost 600, 13px, letter-spacing 0.22em, uppercase, color #C9A84C, opacity 0.7, mb 44px)

  .headline: Playfair Display bold italic, font-size 103px, line-height 1.04, letter-spacing -0.02em,
             color #F2EDE4, max-width 900px
  GERE: headline sobre tese central ou posicionamento, 8–12 palavras. span.gold na expressão central.

  .divider (mt 52px mb 52px): width 100%, height 1px,
             background linear-gradient(90deg, transparent, #C9A84C66, transparent)

  .body-text: Jost 400, font-size 38px, line-height 1.6, color #8A8070, max-width 900px
  GERE: 2–3 frases sobre a tese ou convicção central, sem linguagem de vendas.
...
function buildHTML(post, b, content = null) {
  ...
  const sig       = b.assinatura   || "Vinícius Bohn";
    ...
    body = `${header}${hero}<main style="padding:40px;">
      <div style="display:grid;grid-template-columns:3fr 2fr;gap:24px;">
        <div style="border-radius:8px;background:linear-gradient(145deg,${b.corSecundaria},#100808);border:1px solid ${fmt.border};padding:40px;position:relative;overflow:hidden;">
          <div style="font-family:'DM Mono',monospace;font-size:9px;color:${primary}77;text-transform:uppercase;letter-spacing:.22em;margin-bottom:20px;">POSICIONAMENTO · ${post.pilar.toUpperCase()}</div>
        ...
        <div>${[["Tipo",post.pilar],["Tom","Autoridade · Proximidade"],["Objetivo","Posicionar, não vender"],["Visual","Escuro · premium"],["Métrica","Cliques · DMs · Comentários"]].map(([k,v])=>`<div style="margin-bottom:14px;"><div style="font-size:10px;color:${textDim};font-family:'DM Mono',monospace;text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;">${k}</div><div style="font-size:13px;color:${textMuted};line-height:1.6;">${v}</div></div>`).join("")}</div>
      </div></main>${footer}`;
  }
